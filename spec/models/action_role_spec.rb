...
describe 'permits_access_for class method' do
  before(:each) do
    ...
    @persistent_user_role   = PersistentUserRole.create(:role_id => @role.id, :user_id => @user.id)
    @action_role = ActionRole.create(:action_id => @action.id, :role_id => @role.id)
  end

  it "action is permitted for user's role" do
    ActionRole.permits_access_for(@controller.controller_name, @action.action_name, @user).should == true
  end

  it "action is not permitted for user's role" do
    ActionRole.permits_access_for(@controller.controller_name, 'some_action', @user).should == false
  end

  it "different user tries to access the action" do
    @user = Factory.create(:user, :login => 'another_person')
    ActionRole.permits_access_for(@controller.controller_name, @action.action_name, @user).should == false
  end

  it "user has a role that does not permit access to the requested action" do
    UserRole.delete_all
    @persistent_user_role  = UserRole.create(:role_id => 555, :user_id => @user.id)
    ActionRole.permits_access_for(@controller.controller_name, @action.action_name, @user).should == false
  end
...
describe "permits_access_for class method" do
  ...
  context "when persistent_user_role permits" do
  ...
  context "when persistent_user_role does not permit" do
    it "should not permit if the session_user_role does not permit" do
      @persistent_user_role = PersistentUserRole.create(:role_id => @another_role.id, :user_id => @user.id)
      @session_user_role    = SessionUserRole.create(:role_id => @another_role.id, :user_id => @user.id)
      ActionRole.permits_access_for(@controller.controller_name, @action.action_name, @user).should == false
    end
  ...
  context "when there is no session_user_role" do
    it "should permit if persistent_user_role permits" do
      @persistent_user_role = PersistentUserRole.create(:role_id => @role.id, :user_id => @user.id)
      ActionRole.permits_access_for(@controller.controller_name, @action.action_name, @user).should == true
    end

    it "should not permit if persistent_user_role does not permit" do
      @persistent_user_role = PersistentUserRole.create(:role_id => @another_role.id, :user_id => @user.id)
      ActionRole.permits_access_for(@controller.controller_name, @action.action_name, @user).should == false
    end
  end
